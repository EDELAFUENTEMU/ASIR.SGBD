...
do
        ...
        AuxNombre=$(echo $line | grep -o '\"[a-z]+\"')
        length=$((${#AuxNombre}-2))
        nombre=${AuxNombre:0:$length}
        fecha=${line:10:10}
        hash5=$(echo $nombre $fecha | md5sum)

        #creo la sentencia de insercion con la adv IGNORE para en caso de que pK se repita la ignore. Lo guardo en el script sql
...
rm -f /home/server/intermedio.text
